...
class ContentUpdateService {
  ...
  Future<UpdateCheckResult> checkForUpdates() async {
    ...
    for (final source in ContentConfig.manifestUrls) {
      try {
        ...
        if (response.statusCode != 200) {
          lastError = 'code ${response.statusCode} depuis $source';
          continue;
        }

        final decoded = jsonDecode(utf8.decode(response.bodyBytes));
        if (decoded is! Map<String, dynamic>) {
          lastError = 'manifest invalide depuis $source';
          continue;
        }
    ...
    throw ContentUpdateException(
      'Le catalogue DroneAtlas est momentanément inaccessible. '
      'Vérifie la connexion et assure-toi que GitHub Pages est actif '
      'ou que le dépôt est public. ${lastError ?? ''}',
    );
  }
